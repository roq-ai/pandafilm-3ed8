...
import { useRoqPlatformUserProfiles, useSeriesCount, useMovieCount, useSubscriptionCount } from 'lib/roq';

import { WatchlistListPage as ShowCaseList } from 'pages/watchlists';

function DashboardPage() {
  const { session } = useSession();

  const seriesStat = useSeriesCount();

  const movieStat = useMovieCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Series',
      result: seriesStat?.data,
      isLoading: seriesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Movies',
      result: movieStat?.data,
      isLoading: movieStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Subscriptions',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`watchlists`} passHref legacyBehavior>
                  <Button
