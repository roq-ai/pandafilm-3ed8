...
import { useRoqClient, useProfileFindManyWithCount } from 'lib/roq';
...
export function ProfileListPage(props: ProfileListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['first_name.contains', 'last_name.contains', 'gender.contains', 'location.contains'],
          ...params.filters,
        },
        'profile',
      ),
  ...
  const { data, error, isLoading, mutate } = useProfileFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.profile.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'last_name', header: 'Last Name', accessorKey: 'last_name' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.birth_date ? format(record?.birth_date, 'dd-MM-yyyy') : '',
    },
