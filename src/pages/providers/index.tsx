...
import { useRoqClient, useProviderFindManyWithCount } from 'lib/roq';
...
export function ProviderListPage(props: ProviderListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [, 'movie.count', 'series.count', 'user.count'],
  ...
  const { data, error, isLoading, mutate } = useProviderFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.provider.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('movie', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Movie',
          accessorKey: 'movie',
          cell: ({ row: { original: record } }: any) => record?._count?.movie,
        }
      : null,
