...
import { useProviderFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['account-owner', 'profile-user', 'administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useProviderFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Providers',
      path: isTenantUser ? `/providers/view/${data?.id}` : '/providers',
      entity: 'provider',
    ...
    { name: 'Series', path: '/series', entity: 'series', service: AccessServiceEnum.PROJECT, icon: FiTv },
    {
      name: 'Subscriptions',
      path: '/subscriptions',
      entity: 'subscription',
